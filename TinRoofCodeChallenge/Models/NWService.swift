...
class NWService: NSObject {
    ...
    var delegate: NWServiceDelegate?
    
    
    func makeGETCall(url: EndPoints){
        ...
        let task = session.dataTask(with: urlRequest) { (data, response, error) in
            ...
            do{
                ...
                DispatchQueue.main.async {
                    ...
                    self.delegate?.getPosts(posts: posts)
                    
                    let users = self.createUsersWith(posts: posts)
                    
                    self.delegate?.getUsers(users: users)
